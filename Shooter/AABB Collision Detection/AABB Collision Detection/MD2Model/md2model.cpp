...
bool CMD2Model::UpdateAnimation(animState_t* animState, float fTimePassed, bool dead = false)
{
	if (isPause == false)
	{
		if (!animState->type == animType_t::STAND)
		{
		...
		{
			animState->curr_frame = animState->endframe;
		}
...
void CMD2Model::ResumeAnimation()
{
	isPause = false;
}
