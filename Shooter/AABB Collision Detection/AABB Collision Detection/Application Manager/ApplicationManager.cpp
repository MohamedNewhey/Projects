...
int ApplicationManager::MouseKey = GLFW_MOUSE_BUTTON_8;
...
bool ApplicationManager::InitalizeApplication(int pWindowSizeWidth, int pWindowSizeHeight)
{
	...
	glfwSetMouseButtonCallback(mWindow, &this->MouseButtons);
...
void ApplicationManager::StartMainLoop()
{
	...
	do 
	{
		...
		if (ApplicationManager::MouseKey != GLFW_MOUSE_BUTTON_8)
		{
			mRenderer->HandleMouseButtons(MouseKey);
			ApplicationManager::MouseKey = GLFW_MOUSE_BUTTON_8;
		}
...
void ApplicationManager::MouseButtons(GLFWwindow* window, int button, int action, int mods)
{
	ApplicationManager::MouseKey = button;
	//ApplicationManager::MouseKeyAction = action;
}
